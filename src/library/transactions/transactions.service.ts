...
export class BookTransactionsService {
  ...
  async issue(issueDto: IssueBookDto) {
    ...
    if (!issueDto.studentId && !issueDto.sectionId) {
      throw new BadRequestException(
        'Student ID or Class/Section ID is required',
      );
    }
  ...
  async issueBulk(issueBulkDto: IssueBulkDto) {
    ...
    if (availableCopies.length < issueBulkDto.quantity) {
      throw new BadRequestException(
        `Not enough available copies. Found ${availableCopies.length}, requested ${issueBulkDto.quantity}`,
      );
    }

    const dueDate = new Date(issueBulkDto.dueDate);
    const issueDate = issueBulkDto.issueDate
      ? new Date(issueBulkDto.issueDate)
      : new Date();
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (updateResult.count !== availableCopies.length) {
        throw new BadRequestException(
          'Some copies became unavailable during processing. Please try again.',
        );
      }
  ...
  async return(returnDto: ReturnBookDto) {
    const copy = await this.prisma.bookCopy.findUnique({
      where: {
        tenantId_code: {
          tenantId: returnDto.tenantId,
          code: returnDto.bookCopyCode,
        },
      },
    });
    if (!copy) throw new NotFoundException('Book copy not found');
    ...
    if (!txn)
      throw new NotFoundException(
        'No active transaction found for this book copy',
      );

    const returnDate = returnDto.returnDate
      ? new Date(returnDto.returnDate)
      : new Date();
  ...
  async reportMissing(tenantId: string, transactionId: string) {
    ...
    if (!txn.bookCopyId)
      throw new BadRequestException('Book copy is missing on this transaction');
    ...
    if (parentUserIds.length > 0) {
      ...
      this.notificationsService
        .createNotificationsForUsers({
          ...
          title: 'Library Invoice Generated',
          message: `${studentName} was marked missing ${bookTitle}. Invoice: $${amount.toFixed(2)}.`,
  ...
  async returnBulk(returnBulkDto: {
    tenantId: string;
    copyCodes: string[];
    returnDate?: Date | string;
  }) {
    const results = await Promise.all(
      returnBulkDto.copyCodes.map(
        async (
          code,
          ...
          try {
            await this.return({
              ...
              bookCopyCode: code,
              returnDate: new Date(
                returnBulkDto.returnDate || new Date(),
              ).toISOString(),
            });
  ...
  async getActiveLoansGrouped(params: {
    tenantId: string;
    page?: number;
    pageSize?: number;
    sectionId?: string;
    ...
    const groups = await this.prisma.bookTransaction.groupBy({
      ...
      where,
      _count: { _all: true },
      orderBy: [{ sectionId: 'asc' }, { studentId: 'asc' }, { bookId: 'asc' }],
      skip,
      take: pageSize,
    });
    ...
    const studentIds = [
      ...new Set(groups.map((g) => g.studentId!).filter(Boolean)),
    ];
    const sectionIds = [
      ...new Set(groups.map((g) => g.sectionId!).filter(Boolean)),
    ];
    ...
    const data = await Promise.all(
      groups.map(async (group) => {
        ...
        const student = group.studentId
          ? students.find((s) => s.id === group.studentId)
          : null;
        const section = group.sectionId
          ? sections.find((s) => s.id === group.sectionId)
          : null;
  ...
  async getStats(tenantId: string) {
    ...
    const totalCopies = await this.prisma.bookCopy.count({
      where: { tenantId },
    });
    const issuedCopies = await this.prisma.bookCopy.count({
      where: { tenantId, status: BookStatus.ISSUED },
    });
    ...
    const [topStudentsDetails, topSectionsDetails] = await Promise.all([
      this.prisma.student.findMany({
        where: {
          id: { in: topStudentsGroups.map((g) => g.studentId as string) },
        },
        select: { id: true, firstName: true, lastName: true },
      }),
      this.prisma.section.findMany({
        where: {
          id: { in: topSectionsGroups.map((g) => g.sectionId as string) },
        },
